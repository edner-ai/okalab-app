...
alter table public.payment_method_fields
  add column if not exists field_value_i18n jsonb not null default '{}'::jsonb;

update public.payment_method_fields
set field_value_i18n = jsonb_build_object(
  'es', coalesce(field_value, ''),
  'en', coalesce(field_value, ''),
  'fr', coalesce(field_value, ''),
  'ht', coalesce(field_value, '')
)
where coalesce(field_value_i18n, '{}'::jsonb) = '{}'::jsonb
