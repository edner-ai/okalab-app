...
alter table public.seminar_interest_requests
  add column if not exists invited_by_request_id uuid null references public.seminar_interest_requests(id) on delete set null,
  add column if not exists invited_by_email text null;

create index if not exists seminar_interest_requests_invited_by_request_idx
  on public.seminar_interest_requests (invited_by_request_id, created_at desc);
...
declare
  ...
  v_request_id uuid;
  v_inviter record;
  v_invited_by_request_id uuid := null;
  v_invited_by_email text := null;
begin
  ...
  if v_source_type = 'prelaunch' and p_invited_by_request_id is not null then
    select
      r.id,
      lower(trim(coalesce(r.email, ''))) as email
    into v_inviter
    from public.seminar_interest_requests r
    where r.id = p_invited_by_request_id
      and r.seminar_id = p_seminar_id;

    if found and coalesce(v_inviter.email, '') <> '' and v_inviter.email <> v_email then
      v_invited_by_request_id := v_inviter.id;
      v_invited_by_email := v_inviter.email;
    end if;
  end if;
  ...
  do update set
    ...
    invited_by_request_id = coalesce(public.seminar_interest_requests.invited_by_request_id, excluded.invited_by_request_id),
    invited_by_email = coalesce(public.seminar_interest_requests.invited_by_email, excluded.invited_by_email),
    status = case
...
as $$
begin
  return public.submit_seminar_interest_request(
    p_seminar_id,
    p_full_name,
    p_email,
    p_phone,
...
revoke execute on function public.submit_seminar_interest_request(uuid, text, text, text, text, text, text, text, uuid) from public;
grant execute on function public.submit_seminar_interest_request(uuid, text, text, text, text, text, text, text, uuid) to anon, authenticated;
