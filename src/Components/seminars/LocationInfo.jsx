...
export default function LocationInfo({ seminar }) {
  ...
  const hasPhysicalModality = seminar?.modality === 'presential' || seminar?.modality === 'hybrid';
  const hasLocation = !!(seminar?.location_address || (seminar?.location_lat && seminar?.location_lng));

  if (!seminar || !hasPhysicalModality || !hasLocation) {
    return null;
  }
  ...
  const locationText = seminar.location_address || `${seminar.location_lat}, ${seminar.location_lng}`;
      ...
      <div className="p-6">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-slate-900 mb-1">{t('location', 'Ubicacion')}</h3>
            <p className="text-slate-600 text-sm leading-relaxed break-words">{locationText}</p>
          </div>
        </div>
