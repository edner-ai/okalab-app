...
import {
  getVideoConferencePlatformIcon,
  getVideoConferencePlatformLabel,
...
export default function VideoConferenceInfo({ seminar, isEnrolled }) {
      ...
      <Card className="bg-slate-50 border-dashed border-2 border-slate-200">
        <div className="p-6 text-center">
          ...
          <p className="text-slate-600 text-sm font-medium">
            {t("enrollToAccessVideo", "Inscribete para acceder a los datos de videoconferencia")}
          </p>
        </div>
      </Card>
    );
  ...
  const platformName = getVideoConferencePlatformLabel(seminar, t);
  const platformIcon = getVideoConferencePlatformIcon(seminar.video_conference_platform);
        ...
        <div className="space-y-4">
          {seminar.video_conference_link ? (
            <div>
              <label className="text-[10px] uppercase tracking-wider text-slate-400 font-bold block mb-1.5 ml-1">
                {t("meetingLink", "Link de la reunion")}
              </label>
          ...
          {seminar.video_conference_link ? (
            ...
            >
              ...
              {t("joinMeeting", "Unirse a la reunion")}
            </Button>
          ) : null}
        </div>
      </div>
